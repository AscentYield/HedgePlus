...
interface IHedgePlusMintingEvents {
  /// @notice Event emitted when contract receives ETH
  event Received(address, uint256);

  /// @notice Event emitted when fee is charged
  event ChargeManagementFee(
    address indexed manager,
    uint256 hedgeplus_amount
  );

  /// @notice Event emitted when aBTC is minted
  event Mint(
  ...
  /// @notice Event emitted when hedgeplus is set
  event HedgePlusSet(address indexed hedgeplus);
  ...
  /// @notice Event emitted when cooldown duration is updated
  event CooldownDurationUpdated(uint24 previousDuration, uint24 cooldownDuration);
}
